...
BOOL CPhonebookApp::InitInstance()
{
	...
	if (!oDataSourceProvider.OpenDataSource())
		return FALSE;

	// Register the application's document templates.  Document templates
	//  serve as the connection between documents, frame windows and views
	CMultiDocTemplate* pDocTemplateCities;
	pDocTemplateCities = new CMultiDocTemplate(IDR_CITIES_TYPE,
		RUNTIME_CLASS(CCitiesDocument),
		RUNTIME_CLASS(CChildFrame), // custom MDI child frame
		RUNTIME_CLASS(CCitiesView));
	if (!pDocTemplateCities)
		return FALSE;
	AddDocTemplate(pDocTemplateCities);

	CMultiDocTemplate* pDocTemplatePhoneTypes;
	pDocTemplatePhoneTypes = new CMultiDocTemplate(IDR_PHONE_TYPES_TYPE,
		RUNTIME_CLASS(CPhoneTypesDocument),
		RUNTIME_CLASS(CChildFrame), // custom MDI child frame
		RUNTIME_CLASS(CPhoneTypesView));
	if (!pDocTemplatePhoneTypes)
		return FALSE;
	AddDocTemplate(pDocTemplatePhoneTypes);

	CMultiDocTemplate* pDocTemplatePersons;
	pDocTemplatePersons = new CMultiDocTemplate(IDR_PERSONS_TYPE,
	...
	pDocTemplatePersons->OpenDocumentFile(NULL);
...
int CPhonebookApp::ExitInstance()
{
	...
	CDataSourceProvider& oDataSourceProvider = CDataSourceProvider::GetInstance();
	oDataSourceProvider.GetDataSource().Close();
